...
#######################
#Load data
t1 <- read_csv("../01_data/cache/tribe_bg_full_integrated_combined_FINAL.csv") %>% 
  filter(time == "time 1") %>% 
  dplyr::select(tribe,GEOID = fips, rt) %>% 
  unique() 

t2 <- read_csv("../01_data/cache/tribe_bg_full_integrated_combined_FINAL.csv") %>% 
  filter(time == "time 2") %>% 
...
# 
# t2overlaps <- df2 %>% 
#   dplyr::select(UID,tribe) %>% 
#   group_by(UID) %>% 
#   add_tally() %>% 
#   ungroup() %>% 
#   filter(n > 1)
# 
# write_csv(t2overlaps,"../01_data/t2overlaps.csv")
...
tribes_time1and2_lst <- inner_join(df1 %>% select(tribe,time) %>% unique(),
                                       df2 %>% select(tribe,time) %>% unique(),
                                       by="tribe")
...
The results in Table \ref{tab:heat} suggest that the present-day lands experience 7 more days in excess of 100 degrees per year compared to historical lands. All point estimates yield significant increases, ranging from 5.40 [2.62, 8.18] from the OLS model excluding OTSA to 5.81 [3.30, 8.33] in the primary OLS model with just tribes in both periods.  The coefficients of the Poisson model are not directly comparable to the OLS estimates, but can be transformed for interpretation. The model suggests that historical lands experience about 1.15 (1.15 = exp(0.14)) days a year over 100 degrees, while the present-days lands experience 5.81 (5.81 = exp(0.13)*(exp(1.98)-1)), additional days over 100, which is comparable to the OLS estimates. 
...
write_csv(as_tibble(model.shell[[1]]$cov),"heat_var.csv")
...
model.shell[[1]] <- ols_clustered(dep.var,df=data_long_m1,clust.var = c("tribe"))
write_csv(as_tibble(model.shell[[1]]$cov),"precip_var.csv")
model.shell[[2]] <- ols_clustered(dep.var,df=data_long_m1_full,clust.var = c("tribe"))
model.shell[[3]] <- ols_clustered(dep.var,df=data_long_m2_noOK,clust.var = c("tribe"))
# model.shell[[4]] <- ols_clustered(dep.var,df=data_long_m3_OK,clust.var = c("tribe"))
model.shell[[4]] <- feols_clustered(dep.var,df=data_long_m1)
...
The results in Table \ref{tab:whp} suggest that present-day lands have higher wildfire hazard potential (WHP) compared to historical lands.  The coefficients on Historical indicate that the historical lands have an average WHP index of around 1.36 on a scale of 1 (very low fire hazard potential) to 5 (very high hazard potential). The results indicate that the present-day lands have a higher WHP than the historical lands by 0.41 [0.26, 0.56] points on the index, a 30% increase.  This estimate is robust to several alternative specifications, including the OLS model with all tribes, the OLS model excluding OTSA, and the Tribe Fixed Effects model.
...
model.shell[[1]] <- ols_clustered(dep.var,df=data_long_m1,clust.var = c("tribe"))
write_csv(as_tibble(model.shell[[1]]$cov),"whp_var.csv")
model.shell[[2]] <- ols_clustered(dep.var,df=data_long_m1_full,clust.var = c("tribe"))
model.shell[[3]] <- ols_clustered(dep.var,df=data_long_m2_noOK,clust.var = c("tribe"))
# model.shell[[4]] <- ols_clustered(dep.var,df=data_long_m3_OK,clust.var = c("tribe"))
model.shell[[4]] <- feols_clustered(dep.var,df=data_long_m1)
...
model.shell[[1]] <- ols_clustered(dep.var,df=data_long_m1,clust.var = c("tribe"))
write_csv(as_tibble(model.shell[[1]]$cov),"og_var.csv")
model.shell[[2]] <- ols_clustered(dep.var,df=data_long_m1_full,clust.var = c("tribe"))
model.shell[[3]] <- ols_clustered(dep.var,df=data_long_m2_noOK,clust.var = c("tribe"))
# model.shell[[4]] <- ols_clustered(dep.var,df=data_long_m3_OK,clust.var = c("tribe"))
...
\hline  
\textbf{Mu Function} \\
\hspace{2mm} Historical (Intercept)  & $-0.233^{*}$ & $-2.644^{***}$  & $-2.787^{***}$  &  $-3.871^{***}$& \\
                             & $(-0.472;\ 0.007)$ &  $(-2.892;\ -2.396)$ &  $(-3.027;\ -2.548)$  &  $(-4.036;\ -3.705)$& \\[1ex] 
\hspace{2mm} Present-day Change     & $1.826^{***}$     &   $0.652^{***}$         & $1.106^{***}$   &   $-0.078$& \\
                             & $(1.414;\ 2.238)$   &   $(0.327;\ 0.977)$   &   $(0.778;\ 1.434)$   & $(-0.265;\ 0.109)$ &\\[1ex] 
\textbf{Nu Function}  \\
...
\textbf{Logit}  \\
\hspace{2mm} Historical (Intercept)    &  &     &    &   &  $-3.335^{***}$ \\
                                     &  &   &  &   &  $(-3.639;\ -3.031)$ \\[1ex] 
\hspace{2mm} Present-day Change     &    &    &       &    & $-0.001$   \\
                                    &  &   &   &  & $(-0.389;\ 0.387)$ \\[1ex] 
\hline \\[-1.8ex]
