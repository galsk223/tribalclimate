...
```{r, message = FALSE, warning = FALSE, echo = FALSE,fig.cap = "Density of Variables"}
...
precip_plot <- ggplot(data = subset(df_plot, variable == "Precipitation"), 
  ...
  geom_density_ridges(jittered_points = TRUE, scale = .95, rel_min_height = .01,
                      ...
                      quantile_lines=TRUE, quantile_fun=function(x,...)mean(x),
                      position = position_points_jitter(height = .00, yoffset= .06),
...
drought_plot <- ggplot(data = subset(df_plot, variable == "Drought"), 
  ...
  geom_density_ridges(jittered_points = TRUE, scale = .95, rel_min_height = .01,
                      point_shape = "|", point_size = 3, size = 0.65,
                      quantile_lines=TRUE, quantile_fun=function(x,...)mean(x),
                      position = position_points_jitter(height = .011, yoffset= 1.2),
                      alpha=.5,show.legend = T, na.rm = TRUE) +
  theme_minimal(base_size = 15) +
  theme(legend.position = "bottom", axis.title.y=element_text(family="Helvetica", size=10), 
        axis.title.x=element_text(family="Helvetica", size=12, face="bold"),
...
- Quasi-Binomial: a generalized linear model using a "logit" link function. These models are used for dependent variables bounded between 0 and 1 and with many 0 observations, in this case for the models estimating the portion of types of land. 
...
sg_table(model.shell,
         ...
         column.labels = c("OLS","\\shortstack{OLS \\\\ All Tribes}","\\shortstack{OLS \\\\ Excluding OTSA}","\\shortstack{OLS \\\\ Only OTSA}","Poisson"),
         label = "tab:heat",
         title = "Extreme Heat")
...
sg_table(model.shell,
         dep.name = "Mean PDSI",
         column.labels = c("OLS","\\shortstack{OLS \\\\ All Tribes}","\\shortstack{OLS \\\\ Excluding OTSA}","\\shortstack{OLS \\\\ Only OTSA}","Tribe FE"),
         label = "tab:drought",
         title = "Drought")
```
