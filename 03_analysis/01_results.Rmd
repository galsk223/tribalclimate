...
#######################
#Load data
t0 <- read_csv("../01_data/tribe_bg_full_integrated_combined_FINAL.csv") %>% 
  select(tribe,census_tribe) %>% 
  unique() %>% 
  group_by(tribe) %>% 
  add_tally() %>% 
  ungroup()
...
t1 <- read_csv("../01_data/tribe_bg_full_integrated_combined_FINAL.csv") %>% 
  ...
  filter(!is.na(UID))
t2 <- read_csv("../01_data/clean/00_all_newpolygons.csv") 

join <- left_join(t1,t2,by=c("UID"))
