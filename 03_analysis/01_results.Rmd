...
```{r, message = FALSE, warning = FALSE, echo = FALSE, results='asis'}
land_sum <- read_rds("../01_data/cache/land_sum.rds")

land_area_kable <- kableExtra::kable(land_sum, format = "latex", booktabs = TRUE, 
                                     digits = 0, align="lcc", linesep = "",
                                     caption = "Change in Land Area per Tribe",
                                     format.args = list(big.mark = ",")) %>%
  kableExtra::kable_styling(font_size = 9.5, latex_options = c('hold_position')) %>%
  kableExtra::footnote(general = c("Land area is calculated by tribe and in some cases includes coextensive", "areas where more than one tribe share a location.")) 
...
In Table 4, we see that distributions for each land characteristic are statistically different in historical and present-day land. These findings are robust for all subsets of the data (tribes found in both time periods, all tribes, and tribes found in both time periods, dissagregated to census areas). Aside from the D statistic for Soil Organic Carbon, all D-statistics are significant at p < 0.0006 Bonferroni equivalent at alpha = 0.5. 

Looking at the distribution plots in Figure 1, we see the ways in which the distributions differ between time periods. For instance, extreme heat and wildfire risk both have greater means in the present day, with greater density of tribes at higher levels. Similarly, precipitation, elevation, and ruggedness have lower means in the present day, with greater density of tribes at lower levels. We see uncertain effects in drought and soil organic carbon: drought experiences more variation overall, and soil organic carbon seems to have greater density at lower values, with a small group of tribes having much increased levels in the present-day. We also see more tribes with $0$ values of oil and gas basins, oil production, and gas production, though the change is less certain for tribes with some land with available resources or production between periods. Lastly, it is unclear how the distribution of access to federal lands differs from the historical period to now.
...
The results in Table \ref{tab:heat} suggest that the present-day lands experience 7 more days in excess of 100 degrees per year compared to historical lands. All point estimates yield significant increases, ranging from 6.11 [3.62, 8.6] from the OLS model with all tribes to 7.08 [3.88, 10.27] in the primary OLS model with just tribes in both periods.  The coefficients of the Poisson model are not directly comparable to the OLS estimates. The model suggests that historical lands experience about 1.14 (1.14 = exp(0.13)) days a year over 100 degrees, while the present-days lands experience 7.11 (7.11 = exp(0.13)*(exp(1.98)-1)), additional days over 100, which is comparable to the OLS estimates. 
...
Table \ref{tab:drought} presents the results of the analysis of differences in drought across the historical and present-day lands. Our variable is the drought in each time, in PDSI.  The coefficients on the intercept indicate that historical lands on average were not in drought.  The coefficients on present-day change do not indicate any robust changes in present-day lands. 
...
Table \ref{tab:precip} presents the results of the analysis of precipitation across the historical and present-day lands. The coefficients on Historical across models range from 871.85 mm to 905.79 mm of precipitation each year on historical lands. We see a significant decrease in the precipitation experienced, estimated to be between 177.19 mm less per year, from the OLS model with all tribes, to 259.84 mm less per year, from the fixed effect model. Using the estimates from the primary regression (1), on average tribes experience 24.5% less annual rainfall in present-day lands than they did in historical lands.
...
The results in Table \ref{tab:whp} suggest that present-day lands have higher wildfire hazard potential (WHP) compared to historical lands.  The coefficients on Historical indicate that the historical lands have an average WHP rating of around 1.37 on a scale of 1 (very low fire hazard potential) to 5 (very high hazard potential). The results indicate that the present-day lands have a higher WHP than the historical lands by 0.4 points on the index, a 29% increase.  This estimate is robust to several alternative specifications, including the OLS model with all tribes, the OLS model excluding OTSA, and the Tribe Fixed Effects model.
...
The results in Table \ref{tab:elevation} suggest that the elevation of present-day lands is less than historical lands.  The coefficients on Historical indicate that the historical lands have an average CCE elevation of 750.56 meters. The results indicate that the present-day lands are an average of 148.10 meters lower than the historical lands, a 19% decrease. We find similar results with the OLS model with all tribes, the model excluding OTSA, and the Tribe FE model.
...
The results in Table \ref{tab:tri} suggest that present-day lands are more less rugged than historical lands.  The coefficients on Historical indicate that the historical lands have an average CCE ruggedness index of 10.24. The results indicate that the present-day lands have an average ruggedness index 3.08 points lower than the historical lands, a 30% decrease. This finding is consistent across all model specifications.
...
We evaluate access to oil and gas based on the likelihood that a tribe's CCEs lie over subsurface oil and gas. We estimate decreases in access to subsurface oil and gas in Table \ref{tab:og_basins_ols}, with a decrease of 4% [-0.08,0.0001] in the primary model, and 5% [-0.09,-0.02] in the model excluding OTSA land. Estimating OLS with a binary dependent variable is known as a linear probability model (LPM).  The LPM has two well-known shortcomings: 1) bias when the mass of the distribution lies outside of the unit interval, and 2) heteroskedasticity.  Since we estimate all models with robust standard errors, we correct for heteroskedasticity.  

We estimate a zero inflated beta regression (Table \ref{tab:bezi}) to assess the robustness of the difference between present-day and historical lands. The positive estimated present-day change of the Nu Function suggests that tribes are more likely to have no access to subsurface oil and gas. However the positive estimated present-day change of the Mu function indicates that for tribes that are to have some access to subsurface oil and gas, they are likely to have more in present-day.
...
We compare the portion of land used in oil production between historical and present-day lands in Table \ref{tab:og_oil_ols}, with model specifications providing little evidence that there is change in oil production on present-day land. We see a significant decrease only in the model excluding OTSA of -0.01 [-0.02,-0.002] in production. 

Table \ref{tab:bezi} provides greater granularity on the change in oil production, with tribe land being more likely to not have any oil production (positive Mu Change) and also a greater amount of oil production if a land is to have any oil production (positive Nu Change).
...
We compare the portion of land used in gas production between historical and present-day lands in Table \ref{tab:og_gas_ols}, with model specifications providing little evidence that there is change in gas production on present-day land. We see a significant decrease only in the model excluding OTSA of -0.02 [-0.03,-0.005] in production. 
...
Finally, we study the difference between proximity to federal lands during the historical and present-day periods. We estimate a consistent change across specifications of 0.01 (Table \ref{tab:pad_ols}), indicating that present-day lands have more federal land within 161 km of the area boundary.
