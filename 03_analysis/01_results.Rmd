...
dat_env <- data_long_m1 %>% 
  select(tribe, GEOID, time,
...
# subset to T1 and T2
dat_env_all_t1 <- filter(dat_env_all, time == "time 1")
dat_env_all_t2 <- filter(dat_env_all, time == "time 2")

dat_env_t1 <- filter(dat_env, time == "time 1")
dat_env_t2 <- filter(dat_env, time == "time 2")
```
...
for_summary[[1]] <- data_long_m1 %>% 
  select(`Days over 100F`=heatdays_mean,
         ...
         `Ruggedness`=tri_mean,
         `Soil Organic Carbon`=SOC_perc,
         `Oil and Gas Basin | Portion`=BasinPortion,
         `Oil and Gas Basin | Binary`=BasinBinary,
         `Oil Production`=AllArea_OilPortion,
         `Gas Production`=AllArea_GasPortion,
         `Fraction Federal Land`=PADPortion) %>%
...
```{r, message = FALSE, warning = FALSE, echo = FALSE, results='asis'}
# Precipitation
ks_precip <- ks.test(dat_env_all_t1$Precipitation, dat_env_all_t2$Precipitation)
ks_precip2 <- ks.test(dat_env_t1$Precipitation, dat_env_t2$Precipitation)

# Presence of oil/gas basin (binary)
ks_og_basin <- ks.test(dat_env_all_t1$"Oil and Gas Basin", dat_env_all_t2$"Oil and Gas Basin")
ks_og_basin2 <- ks.test(dat_env_t1$"Oil and Gas Basin", dat_env_t2$"Oil and Gas Basin")
ks_og_basinbinary <- ks.test(dat_env_all_t1$"Oil and Gas Basin | Binary", dat_env_all_t2$"Oil and Gas Basin | Binary")
ks_og_basinbinary2 <- ks.test(dat_env_t1$"Oil and Gas Basin | Binary", dat_env_t2$"Oil and Gas Basin | Binary")

# Oil Production
ks_oil <- ks.test(dat_env_all_t1$"Oil Production", dat_env_all_t2$"Oil Production")
ks_oil2 <- ks.test(dat_env_t1$"Oil Production", dat_env_t2$"Oil Production")

# Gas production
ks_gas <- ks.test(dat_env_all_t1$"Gas Production", dat_env_all_t2$"Gas Production")
ks_gas2 <- ks.test(dat_env_t1$"Gas Production", dat_env_t2$"Gas Production")

# Heat
ks_heat <- ks.test(dat_env_all_t1$"Days over 100F", dat_env_all_t2$"Days over 100F")
ks_heat2 <- ks.test(dat_env_t1$"Days over 100F", dat_env_t2$"Days over 100F")

# Drought
ks_drought <- ks.test(dat_env_all_t1$Drought, dat_env_all_t2$Drought)
ks_drought2 <- ks.test(dat_env_t1$Drought, dat_env_t2$Drought)

# Wildfire risk
ks_fire <- ks.test(dat_env_all_t1$"Wildfire Hazard Potential", dat_env_all_t2$"Wildfire Hazard Potential")
ks_fire2 <- ks.test(dat_env_t1$"Wildfire Hazard Potential", dat_env_t2$"Wildfire Hazard Potential")

# Elevation
ks_elevation <- ks.test(dat_env_all_t1$Elevation, dat_env_all_t2$Elevation)
ks_elevation2 <- ks.test(dat_env_t1$Elevation, dat_env_t2$Elevation)

# TRI (ruggedness)
ks_tri <- ks.test(dat_env_all_t1$Ruggedness, dat_env_all_t2$Ruggedness)
ks_tri2 <- ks.test(dat_env_t1$Ruggedness, dat_env_t2$Ruggedness)

# Fraction federal lands
ks_fed <- ks.test(dat_env_all_t1$"Fraction Federal Land", dat_env_all_t2$"Fraction Federal Land")
ks_fed2 <- ks.test(dat_env_t1$"Fraction Federal Land", dat_env_t2$"Fraction Federal Land")

# SOC
ks_soc <- ks.test(dat_env_all_t1$"Soil Organic Carbon", dat_env_all_t2$"Soil Organic Carbon")
ks_soc2 <- ks.test(dat_env_t1$"Soil Organic Carbon", dat_env_t2$"Soil Organic Carbon")
...
ks_results <- tibble(
  ...
  D_sub = c(paste(round(ks_heat2$statistic, digits = 3), "***"),
            ...
            paste(round(ks_og_basinbinary2$statistic, digits = 3), "***"),
  ...
  D_all = c(paste(round(ks_heat$statistic, digits = 3), "***"),
            ...
            paste(round(ks_og_basinbinary$statistic, digits = 3), "***"),
...
kable_ks_tests <- kable(ks_results, format = "latex", booktabs = TRUE, digits = 3, align="c", linesep = "",
                        caption = "Kolmogorov-Smirnov Tests",
                        col.names = c("Variable", "D Statistic", "D-Statistic")) %>%
  kableExtra::kable_styling(font_size = 9.5) %>%
  kableExtra::add_header_above(header = c(" "=1,"Tribes with \nT1 and T2 Landholdings"=1,  "All Tribes"=1),
                   align = "c",
                   bold = TRUE) %>%
  kableExtra::footnote(general = c("*p < 0.1; **p < 0.05; ***p < 0.01")) %>%
...
texreg::screenreg(list(bezi_model1,bezi_model1_full,bezi_model2_noOK,bezi_model3_OK),
       ...
       # custom.model.names = c("BEZI","BEZI | All Tribes","BEZI | Excluding OK", "BEZI | Only OK", "Logit"),
       digits = 3,
...
Historical (Intercept)    & $1.195^{***}$ &   $1.155 ^{***}$        & $1.150 ^{***}$   &   $1.297 ^{***}$               \\
                     & $(1.146;\ 1.244)$ &   $(1.111;\ 1.198)$ &   $(1.090;\ 1.210)$    &   $(1.235;\ 1.360)$          \\ \ [1ex]
Present-day Change     & $0.240.$     &   $0.326^{**}$         & $0.363 ^{*}$   &   $0.028$              \\
                     & $(-0.016;\ 0.497)$   &   $(0.094;\ 0.557)$   &   $(0.038;\ 0.687)$   & $(-0.387;\ 0.442)$            \\ [1ex]
...
model.shell[[1]] <- glm_clustered(dep.var, family = "binomial", df = data_long_m1)
model.shell[[2]] <- glm_clustered(dep.var, family = "binomial", df = data_long_m1_full)
model.shell[[3]] <- glm_clustered(dep.var, family = "binomial", df = data_long_m2_noOK)
model.shell[[4]] <- glm_clustered(dep.var, family = "binomial", df = data_long_m3_OK)

sg_table(model.shell,
         dep.name = "Dummy for Land With Subsurface Oil and Gas",
         label = "tab:soc",
         column.labels = c("Logit","Logit | All Tribes","Logit | Excluding OK","Logit | Only OK"),
         title = "Tribes with Land With Subsurface Oil and Gas")
