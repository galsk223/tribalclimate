...
For historical time periods, we use the counties as described in the manuscript. In the present-day time periods, we use census native areas polygons (U.S. Census Bureau, 2018).  While multiple tribes may have occupied the same county area in the historical period, the present day boundaries are mutually exclusive. We aggregate across counties and census native areas to produce a dataset where the unit of observation is tribe and time period. In the case of gridded variables of land characteristics, the data consist of observations for grid cells that fall within land area polygons. When we calculate drought we weight each value of a cell by the portion of cell area that falls within a census area polygon. Additionally, when we aggregate census areas up to tribes, we weight by census area. While we do not weight the cell values of precipitation and wildfire hazard potential, we weight the census area polygon when we aggregate. The rest of the land characteristic variables are unweighted both on the cell level (if gridded) and on the census area level. 
...
## Ruggedness
We build a ruggedness index based on the digital elevation model. We refer to the *raster* package in R (Hijmans, 2020), which states that the Terrain Ruggedness Index is “... the mean of the absolute differences between the value of a cell [elevation] and the value of its 8 surrounding cells (Wilson et al., 2007).”  The ruggedness is calculated at approximately 100 meter resolution for every county or census native area. For each polygon, we extract the mean Terrain Ruggedness Index for all of a tribe's lands for consistency with other measurements in the analysis.
...
## Federal Lands
The U.S. Geological Survey's Protected Lands Database of the U.S. (PAD-US) is a spatial inventory of all formally protected lands (U.S. Geological Survey (USGS) Gap Analysis Project (GAP), 2018). We select only lands protected by the Federal Government (e.g. national parks, national forests, etc.). We buffer the tribal land to include the land within 160 km (~100 miles), and find the portion of land federally protected within the buffered polygon. 

## Final Dataset Construction
Each variable described above is constructed at the county (historical) and census native area (present-day).  These units are not comparable.  We aggregate the dataset to the tribe level in historical and present-day by calculating the mean of the counties (or census native areas) associated with a tribe.
...
Figure 1 depicts the difference in the distributions between time periods. For instance, extreme heat and wildfire risk both have greater means in the present day, with greater density of tribes at higher levels. Similarly, precipitation, elevation, and ruggedness have lower means in the present day, with greater density of tribes at lower levels. The mean of drought is very similar in the historical and present-day periods.  However, the variance is much higher in present day.  Similarly, soil organic carbon has slightly more distribution mass at very low levels and some mass at higher levels.  The distribution of oil and gas basins in the present-day period is more concentrated at $0$ and $1$ when compared with the historical period.  Lastly, the oil and gas production as well as the fraction of federal lands variables are heavily concentrated on $0$ with a small number of tribes in present-day with much higher portions of land with oil and gas wells and federal lands.
...
The results in Table \ref{tab:heat} suggest that the present-day lands experience 7 more days in excess of 100 degrees per year compared to historical lands. All point estimates yield significant increases, ranging from 6.11 [3.62, 8.6] from the OLS model with all tribes to 7.08 [3.88, 10.27] in the primary OLS model with just tribes in both periods.  The coefficients of the Poisson model are not directly comparable to the OLS estimates, but can be transformed for interpretation. The model suggests that historical lands experience about 1.14 (1.14 = exp(0.13)) days a year over 100 degrees, while the present-days lands experience 7.11 (7.11 = exp(0.13)*(exp(1.98)-1)), additional days over 100, which is comparable to the OLS estimates. 
...
Table \ref{tab:drought} presents the results of the analysis of differences in drought across the historical and present-day lands. Our variable is the drought in each time, in PDSI.  Negative values indicate drought while positive values indicate wetter conditions.  The coefficients on the intercept indicate that historical lands on average were not in drought 0.02 [0.01, 0.02].  Using data on tribes in both historical and present-day, we find no significant difference between the means in the two periods (0.002 [-0.01, 0.01]).  Including all tribes suggests that conditions became less prone to drought. The coefficients on present-day are not robust across models. 
...
sg_table(model.shell,
         dep.name = "Mean PDSI (index -10 to 10)",
...
The results in Table \ref{tab:whp} suggest that present-day lands have higher wildfire hazard potential (WHP) compared to historical lands.  The coefficients on Historical indicate that the historical lands have an average WHP index of around 1.37 on a scale of 1 (very low fire hazard potential) to 5 (very high hazard potential). The results indicate that the present-day lands have a higher WHP than the historical lands by 0.4 [0.22, 0.57] points on the index, a 29% increase.  This estimate is robust to several alternative specifications, including the OLS model with all tribes, the OLS model excluding OTSA, and the Tribe Fixed Effects model.
...
The results in Table \ref{tab:tri} suggest that present-day lands are less rugged than historical lands.  The coefficients on Historical indicate that the historical lands have an average CCE ruggedness index of 10.24, which is generally considered flat. The results indicate that the present-day lands have an average ruggedness index 3.08 points lower than the historical lands, a 30% decrease. This finding is consistent across all model specifications.
...
Finally, we study the difference between proximity to federal lands during the historical and present-day periods. We estimate a consistent change across specifications of 0.01 [0.001, 0.02] (Table \ref{tab:pad_ols}), indicating that present-day lands have more federal land within 161 km of the area boundary.  This result is consistent across specifications, but is not significant at the Bonferroni adjusted cutoff.
